...
export class GardenersController {
  ...
  @ApiResponse({ status: 201, description: "Gardener profile created successfully" })
  ...
  @Roles(UserRole.GARDENER)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Get all invitations received by this gardener" })
  @ApiResponse({ status: 200, description: "Invitations retrieved successfully" })
  async getMyInvitations(@Request() req) {
    return this.gardenersService.getGardenerInvitations(req.user.id);
  }
  ...
  @ApiResponse({ status: 200, description: "Left nursery successfully" })
  async leaveNursery(@Request() req) {
    return this.gardenersService.leaveNursery(req.user.id);
  }

  // ─── Public static routes (BEFORE /:gardener_id) ───────────────────────────
  ...
  @ApiResponse({ status: 200, description: "Gardener profile retrieved successfully" })
