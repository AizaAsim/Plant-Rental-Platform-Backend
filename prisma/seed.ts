...
async function main() {
  ...
  const nursery1 = await prisma.nursery.create({
    data: {
      ...
      profilePictureUrl: IMG.nurseryLogo,
  ...
  await prisma.freelanceMatchConfig.upsert({
    where: { id: "singleton" },
    update: {},
    create: {
      id: "singleton",
  ...
  const couponRent = await prisma.coupon.upsert({
    where: { code: "RENT10" },
    update: {},
    create: {
      code: "RENT10",
...
📦 Sample orders: ORD-SEED-1001 (completed rent), 1002 (buy), 1003 (awaiting payment), 1004 (active rent), 1005 (overdue + penalty ~PKR ${Number(penaltyCalc.runningTotal)})
   Penalty test: customer1@example.com → GET /api/v1/orders/ORD-SEED-1005/penalty
📅 Booking: BKG-SEED-2001 | Task: TSK-SEED-3001 | Jobs: fj-seed-open-01, fj-seed-done-01
💬 Chat session: ${chatSession.id}

♻️  Re-seed orders only (no full wipe): SEED_MODE=scenarios npx prisma db seed
`);
}
